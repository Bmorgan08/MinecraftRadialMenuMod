...
public class RadialMenuConfigScreen extends Screen {
    ...
    protected void init() {
        allBindings = Arrays.stream(minecraft.options.keyMappings)
                .sorted((a, b) -> {
                    int cat = a.getCategory().id().toString().compareTo(b.getCategory().id().toString());
    ...
    public void render(GuiGraphics gfx, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (bind != null && !bind.isEmpty()) {
                KeyMapping km = KeyMapping.get(bind);
        ...
        if (editingSlot >= 0) {
            ...
            for (int i = scrollOffset; i < allBindings.size() && visibleRow < LIST_ROWS; i++) {
                ...
                if (!cat.equals(lastCategory)) {
                    ...
                    gfx.drawString(font, km.getCategory().label().getString(), rightX + 3, ry + 4, 0xFFAAAAFF);
                    lastCategory = cat;
                    visibleRow++;
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean handled) {
        ...
        // Forward clicks to the name box first
        if (nameBox.visible && nameBox.mouseClicked(event, false)) {
            setFocused(nameBox);
            return true;
        }
    ...
    @Override
    public void onClose() {
        minecraft.setScreen(new RadialMenuScreen(hud));
    }
}
